...
@BotSingleton.bot.message_handler(func=lambda message: message.text == "Изменить запись")
...
@BotSingleton.bot.message_handler(func=lambda message: all([message.text in weeks_days,user_states.get(message.chat.id) == "change_workout"]))
def handle_selected_change_day(message):
    if message.text == "Назад":
        ...
        BotSingleton.bot.send_message(user_id,
                        f"Какой слот вы хотите изменить",
                        reply_markup=get_day_slots_markup(slots=example_slots))


@BotSingleton.bot.message_handler(func=lambda message: all([validate_time(message.text),user_states.get(message.chat.id) == "selected_day_changing_slot"]))
...
@BotSingleton.bot.message_handler(func=lambda message: all([message.text == "Изменить время",user_states.get(message.chat.id) == "selected_day_changing_slot"]))
def handle_change_workout_time(message):
    user_states[user_id] = "start"
    BotSingleton.bot.send_message(user_id, "Выберите опцию:", reply_markup=get_start_markup())

@BotSingleton.bot.message_handler(func=lambda message: all([message.text == "Удалить тренировку",user_states.get(message.chat.id) == "selected_day_changing_slot"]))
def handle_delete_workout(message):
    user_states[user_id] = "start"
    BotSingleton.bot.send_message(user_id, "Выберите опцию:", reply_markup=get_start_markup())

@BotSingleton.bot.message_handler(func=lambda message: all([message.text == "Освободить слот и уведомить пользователей",
